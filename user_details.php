...
if (isset($_REQUEST["flg"]) && $_REQUEST["flg"] == "del") {
    try {
        ...
        $stmt_del = $obj->con1->prepare("DELETE FROM `registration` WHERE id=?");
        $stmt_del->bind_param('i', $reg_id);
...
<div class='p-6 animate__animated' x-data='pagination'>
    <h1 class="dark:text-white-dar  pb-8 text-3xl font-bold">User Details</h1>
    <div class="panel mt-6 flex items-center  justify-between relative">
        <button type="button" class="p-2 btn btn-primary m-1 add-btn" onclick="javascript:insertdata()">
            <i class="ri-add-line mr-1"></i> Add New User</button>
...
<script>
        ...
        Alpine.data('pagination', () => ({
            ...
            init() {
                this.datatable = new simpleDatatables.DataTable('#myTable', {
                    data: {
                        headings: ['Sr.No.', 'Name', 'Phone Number', 'Actions'],
                        data: [
                            <?php
                            $stmt = $obj->con1->prepare(
                                "SELECT *,concat(concat(firstname,' '),lastname) as 'fullname' FROM `registration` order by id desc;"
                            );
                            $stmt->execute();
    ...
    function insertdata(id){
        eraseCookie("edit_id");
        eraseCookie("view_id");
        window.location = "tribal_registration.php";
    }
    ...
    async function showAlert(id) {
        new window.Swal({
            ...
            if (result.isConfirmed) {
                var loc = "user_details.php?flg=del&reg_id=" + id;
                window.location = loc;
            }
        });
    }
