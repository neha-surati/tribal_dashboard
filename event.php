...
if (isset($_REQUEST["flg"]) && $_REQUEST["flg"] == "del") {
    ...
    try {
        ...
        while ($row_subimg = mysqli_fetch_array($Resp_subimg)) {
            if (file_exists("images/event_image/" . $row_subimg["img"])) {
                unlink("images/event_image/" . $row_subimg["img"]);
            }
        }
        ...
        $stmt_del = $obj->con1->prepare("DELETE FROM `event` WHERE event_id=?");
    ...
    if ($Resp) {
        if (file_exists("images/event_image/" . $event_img)) {
            unlink("images/event_image/" . $event_img);
        }
...
<script type="text/javascript">
        ...
        Alpine.data('pagination', () => ({
            ...
            init() {
                this.datatable = new simpleDatatables.DataTable('#myTable', {
                    data: {
                        headings: ['Sr.No.', 'Event', 'Date', 'Action'],
                        data: [
                            <?php
    ...
    async function showAlert(id, event_img) {
        new window.Swal({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                var loc = "event.php?flg=del&event_id=" + id + "&event_img=" + event_img;
                window.location = loc;
            }
        });
    }
