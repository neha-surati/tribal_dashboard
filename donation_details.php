...
if (isset($_REQUEST["flg"]) && $_REQUEST["flg"] == "del") {
    try {
        ...
        $stmt_del = $obj->con1->prepare("DELETE FROM `donation` WHERE id=?");
...
<div class='p-6 animate__animated' x-data='pagination'>
    <h1 class="dark:text-white-dar  pb-8 text-3xl font-bold">Donation Details</h1>
    <div class="panel mt-6 flex items-center  justify-between relative">
        <button type="button" class="p-2 btn btn-primary m-1 add-btn" onclick="javascript:insertdata()">
            <i class="ri-add-line mr-1"></i>New Donation</button>
                    ...
                    data: {
                        ...
                        data: [
                            ...
                            $stmt = $obj->con1->prepare(
                                "SELECT *,concat(concat(firstname,' '),lastname) as 'fullname' FROM `donation` order by id desc;"
                            );
                            $stmt->execute();
    ...
    async function showAlert(id) {
        new window.Swal({
            ...
            if (result.isConfirmed) {
                var loc = "donation_details.php?flg=del&reg_id=" + id;
                window.location = loc;
            }
        });
    }
