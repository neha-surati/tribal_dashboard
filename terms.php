...
$stmt = $obj->con1->prepare("SELECT * FROM `terms`");
$stmt->execute();
...
if (isset($_REQUEST['update'])) {
    ...
    try {
        $stmt = $obj->con1->prepare("UPDATE `terms` SET `description`=?,`status`=?");
...
if (isset($_REQUEST["save"])) {
    ...
    $status = (isset($_REQUEST["status"])) ? "enable" : "disable";
    try {
        $stmt = $obj->con1->prepare("INSERT INTO `terms`(`description`, `status`) VALUES (?,?)");
        $stmt->bind_param("ss", $desc, $status);
        $Resp = $stmt->execute();
        if (!$Resp) {
            throw new Exception(
...
<div class='p-6'>
    <div class="flex gap-6 items-center pb-8">
        ...
        <h1 class="dark:text-white-dar text-3xl font-bold">Terms and Conditions</h1>
    </div>
